...
export default function HatcheryPage() {
  ...
  // Auto-hatch once the timer hits zero.
  // (Keeps your big "Hatch" button too, but you won’t get stuck babysitting it.)
  const [autoHatched, setAutoHatched] = useState(false);

  // If a different egg ever lands in slot 1 later, allow auto-hatch again.
  useEffect(() => {
    setAutoHatched(false);
  }, [selectedEgg?.id]);

  useEffect(() => {
    if (!selectedEgg) return;
    if (autoHatched) return;
    if (!data?.hatch?.ready) return;
    ...
    (async () => {
      try {
        ...
        // If something flaked (network), just let the user hit the button.
        console.warn("auto-hatch failed:", e);
      }
    })();
  ...
  }, [selectedEgg, autoHatched, data?.hatch?.ready, navigate]);
