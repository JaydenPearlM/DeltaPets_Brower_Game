...
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    SELECT
      NULL::public.pets,
      false,
      'Egg not found or already hatched';
  ...
  /*
   * Give the newly hatched Kith one Evolution Mark
   * in its native elemental line.
   *
   * This preserves any existing higher values.
   */
  ...
  VALUES (
    ...
    CASE
      WHEN p_line::text = 'water' THEN 1
      ELSE 0
    END,

    CASE
      WHEN p_line::text = 'fire' THEN 1
      ELSE 0
    END,

    CASE
      WHEN p_line::text = 'earth' THEN 1
      ELSE 0
    END,

    CASE
      WHEN p_line::text = 'air' THEN 1
      ELSE 0
    END,

    CASE
      WHEN p_line::text = 'ice' THEN 1
      ELSE 0
    END,

    CASE
      WHEN p_line::text = 'storm' THEN 1
      ELSE 0
    END,

    CASE
      WHEN p_line::text = 'light' THEN 1
      ELSE 0
    END,

    CASE
      WHEN p_line::text = 'shadow' THEN 1
      ELSE 0
    END
  )
  ...
  DO UPDATE SET
    ...
    water = GREATEST(
      public.pet_elements.water,
      EXCLUDED.water
    ),

    fire = GREATEST(
      public.pet_elements.fire,
      EXCLUDED.fire
    ),

    earth = GREATEST(
      public.pet_elements.earth,
      EXCLUDED.earth
    ),

    air = GREATEST(
      public.pet_elements.air,
      EXCLUDED.air
    ),

    ice = GREATEST(
      public.pet_elements.ice,
      EXCLUDED.ice
    ),

    storm = GREATEST(
      public.pet_elements.storm,
      EXCLUDED.storm
    ),

    light = GREATEST(
      public.pet_elements.light,
      EXCLUDED.light
    ),

    shadow = GREATEST(
      public.pet_elements.shadow,
      EXCLUDED.shadow
    );
